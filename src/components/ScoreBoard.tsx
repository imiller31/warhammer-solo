...
export function ScoreBoard({
  ...
  return (
    <div className="gd-panel-riveted rounded-lg p-3 sm:p-4">
      <h2 className="text-lg font-gothic font-bold gd-gold mb-3 tracking-wider">Score</h2>
      <div className="grid grid-cols-2 gap-3 sm:gap-4">
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <span className="text-xl font-bold gd-brass-counter w-8 text-center">{playerVP}</span>
            <button onClick={() => onAdjustVP('player', 1)} className="gd-btn w-11 h-11 rounded text-xs font-bold">+</button>
          </div>
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <span className="text-xl font-bold gd-brass-counter w-8 text-center">{playerCP}</span>
            <button onClick={() => onAdjustCP('player', 1)} className="gd-btn w-11 h-11 rounded text-xs font-bold">+</button>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <span className="text-xl font-bold gd-brass-counter w-8 text-center">{aiVP}</span>
            <button onClick={() => onAdjustVP('ai', 1)} className="gd-btn w-11 h-11 rounded text-xs font-bold">+</button>
          </div>
          <div className="flex items-center gap-1 sm:gap-2">
            <span className="text-xs gd-bone opacity-40 w-6 font-gothic">CP</span>
            <button onClick={() => onAdjustCP('ai', -1)} className="gd-btn w-11 h-11 rounded text-xs font-bold">−</button>
            <span className="text-xl font-bold gd-brass-counter w-8 text-center">{aiCP}</span>
            <button onClick={() => onAdjustCP('ai', 1)} className="gd-btn w-11 h-11 rounded text-xs font-bold">+</button>
          </div>
        </div>
      </div>
